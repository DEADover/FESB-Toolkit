      ...
      {entries.length > 0 && (
        <div className="shrink-0 px-3 py-1 border-b border-t-line bg-t-panel flex items-center gap-2">
          ...
          {filtered.length !== entries.length && (
            <span className="text-[11.5px] text-t-ink4 shrink-0">{filtered.length} / {entries.length}</span>
          )}
        </div>
      )}
  ...
  return (
    <button onClick={onClick}
      className={`w-full text-left flex flex-col gap-0.5 px-3 py-2 border-b border-t-line/40 transition-colors border-l-2 border-l-transparent ${
        selected ? "bg-accent/10" : "hover:bg-t-hover/50"
      }`}>
      <div className="flex items-center gap-2 text-[11.5px]">
        {entry.is_file
          ? <FileText className="w-3 h-3 text-caution shrink-0" />
      ...
      <div className="flex items-center gap-2 text-[10.5px] pl-5">
        ...
        <span className="px-1 rounded-md bg-accent/15 text-accent font-mono font-medium truncate" title={`Queue: ${entry.address}`}>
          {entry.address}
        </span>
        {propCount > 0 && (
        ...
        {entry.is_file && entry.file_name && (
          <span className="text-caution font-mono truncate" title={`File: ${entry.file_name}`}>
            {entry.file_name}
          </span>
        )}
      </div>
  ...
  return (
    <div className="flex-1 min-w-0 flex flex-col min-h-0 overflow-hidden select-text">
      ...
      <div className="shrink-0 px-3 py-1.5 border-b border-t-line bg-t-panel flex items-center gap-2">
        {entry.is_file
          ? <FileText className="w-3.5 h-3.5 text-caution shrink-0" />
          : <Mail     className="w-3.5 h-3.5 text-t-ink4 shrink-0" />}
        <span className="text-[12.5px] text-t-ink font-mono truncate" title={entry.id}>{entry.id}</span>
        <span className="text-[11.5px] text-t-ink5 font-mono shrink-0">{entry.timestamp}</span>
        <span className="text-[10.5px] px-1 py-0.5 rounded-md bg-accent/15 text-accent font-mono shrink-0" title={`Queue: ${entry.address}`}>
          {entry.address}
        </span>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        <div className="flex items-center gap-2 text-[11.5px] flex-wrap">
          <span className="text-[10.5px] px-1.5 py-0.5 rounded-md bg-t-hover text-t-ink3 font-medium uppercase">
            {entry.is_file ? "binary" : detected}
          </span>
          {bodyText && <span className="text-t-ink5 font-mono">{fmtBytes(bodyBytes)}</span>}
          {entry.is_file && entry.file_name && (
            <span className="text-caution font-mono">{entry.file_name}</span>
          )}
          {entry.is_file && (
            <span className={`font-mono ${entry.file_data_b64 ? "text-positive" : "text-t-ink5"}`}>
        ...
        >
          {hasProps
            ...
            : <p className="text-[11.5px] text-t-ink5">{t("history.props.none")}</p>
          }
        </CollapsibleSection>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div onClick={e => e.stopPropagation()}
        ...
        <div className="shrink-0 px-4 py-2.5 border-b border-t-line bg-t-panel flex items-center gap-2">
          ...
          <span className="text-[11.5px] text-t-ink5">
        ...
        <div className="shrink-0 grid grid-cols-2 gap-px bg-t-line border-b border-t-line">
          <div className="bg-t-panel px-3 py-1.5">
            <div className="flex items-center gap-2">
              <span className="text-[10.5px] uppercase tracking-wider text-accent font-bold">{t("history.diff.left")}</span>
              <span className="text-[11.5px] font-mono text-t-ink2 truncate">{left.id}</span>
            </div>
          ...
          <div className="bg-t-panel px-3 py-1.5">
            <div className="flex items-center gap-2">
              <span className="text-[10.5px] uppercase tracking-wider text-caution font-bold">{t("history.diff.right")}</span>
              <span className="text-[11.5px] font-mono text-t-ink2 truncate">{right.id}</span>
            </div>
            <div className="flex items-center gap-2 mt-0.5 text-[10.5px] text-t-ink5 font-mono">
        ...
        {/* Body diff */}
        <div className="flex-1 overflow-y-auto font-mono text-[11.5px] leading-5">
          {ops.map((op, i) => {
            const bg =
              op.kind === "del" ? "bg-negative/10" :
              op.kind === "add" ? "bg-positive/10" :
                                  "";
            const sign =
              op.kind === "del" ? "−" :
              op.kind === "add" ? "+" : " ";
            const text =
              ...
              op.kind === "add" ? op.right :
                                  op.left;
            const align =
              op.kind === "del" ? "text-negative"   :
              op.kind === "add" ? "text-positive" :
                                  "text-t-ink3";
            return (
        ...
        <div className="shrink-0 px-3 py-1.5 border-t border-t-line bg-t-panel flex items-center text-[10.5px] text-t-ink5">
          <span>{t("history.diff.note")}</span>
          <span className="ml-auto flex items-center gap-1">
            <kbd className="font-mono px-1 py-0.5 border border-t-line rounded-md">Esc</kbd> {t("history.diff.close")}
          </span>
        </div>
      </div>
    </div>
