  ...
  return (
    ...
    >
      <div onClick={e => e.stopPropagation()}
        className="bg-t-bg border border-t-line rounded-xl shadow-2xl w-[640px] max-w-[90vw] max-h-[70vh] flex flex-col overflow-hidden">
        ...
        <div className="shrink-0 flex items-center gap-2 px-3 py-2 border-b border-t-line bg-t-panel">
          ...
          />
          <kbd className="text-[10.5px] text-t-ink5 font-mono px-1.5 py-0.5 border border-t-line rounded-md">Esc</kbd>
          <button onClick={onClose} className="p-1 rounded-md text-t-ink4 hover:text-t-ink hover:bg-t-hover" aria-label={t("palette.close")}>
        ...
        <div ref={listRef} className="flex-1 overflow-y-auto">
          {filtered.length === 0 ? (
            <p className="text-[12.5px] text-t-ink5 text-center py-8">{t("palette.nothing")}</p>
          ) : groups.map(g => (
            <div key={g.category}>
              {g.category && (
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2 text-left transition-colors ${
                      isActive ? "bg-accent/15" : "hover:bg-t-hover/50"
                    } ${a.disabled ? "opacity-40 cursor-not-allowed" : ""}`}
                  >
                    {a.icon && <span className="shrink-0 text-t-ink4">{a.icon}</span>}
                    <div className="flex-1 min-w-0">
                      <div className={`text-[13px] truncate ${isActive ? "text-accent" : "text-t-ink"}`}>
                        {a.label}
                      </div>
                      {a.hint && (
                        <div className="text-[10.5px] text-t-ink5 truncate">{a.hint}</div>
                      )}
                    </div>
                    {a.kbd && (
                      <kbd className="text-[10.5px] text-t-ink5 font-mono px-1.5 py-0.5 border border-t-line rounded-md shrink-0">
                        {a.kbd}
                      </kbd>
                    )}
                    {isActive && !a.kbd && <ArrowRight className="w-3 h-3 text-accent shrink-0" />}
                  </button>
                );
              })}
            </div>
        ...
        <div className="shrink-0 px-3 py-1.5 border-t border-t-line bg-t-panel flex items-center gap-3 text-[10.5px] text-t-ink5">
          <span className="flex items-center gap-1">
            <kbd className="font-mono px-1 py-0.5 border border-t-line rounded-md">↑↓</kbd> navigate
          </span>
          <span className="flex items-center gap-1">
            <kbd className="font-mono px-1 py-0.5 border border-t-line rounded-md">↵</kbd> run
          </span>
          <span className="flex items-center gap-1">
