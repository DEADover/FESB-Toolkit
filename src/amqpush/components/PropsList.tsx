 ...
 * mono `text-[11.5px]`, with a hover-revealed Copy button per value.
 *
...
export default function PropsList({ items, onLog }: {
  ...
  if (visible.length === 0) return <p className="text-[11.5px] text-t-ink5">—</p>;
  return (
    <div className="text-[11.5px] font-mono select-text">
      {visible.map(([k, v]) => {
        const value = String(v);
        return (
          <div key={k}
            className="group grid grid-cols-[140px_1fr_auto] gap-x-3 items-start py-0.5 px-1 -mx-1 rounded-md hover:bg-t-hover/50 transition-colors">
            ...
            <CopyButton
              ...
              title={`Copy ${k}`}
              className="opacity-0 group-hover:opacity-100 transition-opacity p-0.5 rounded-md text-t-ink5 hover:text-t-ink2 hover:bg-t-hover"
            />
          </div>
        );
      })}
